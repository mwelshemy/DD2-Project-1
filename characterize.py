...
my_cells = ['invx1', 'invx2', 'invx4', 'invx8', 'nand2x1', 'nand2x2', 'nand2x4']
...
VDD VDD 0 1.8
VIN IN 0 PULSE(1.8 0 1n {slew_ns}n {slew_ns}n 2n 6n)
...
"""
    ...
    for line in result.stdout.split('\n'):
        if line.startswith('cell_rise =') or line.startswith('cell_fall =') or line.startswith('rise_transition =') or line.startswith('fall_transition ='):
            parts = line.split('=')
            results[parts[0].strip()] = float(parts[1].strip())
            
    # LIVE PROGRESS UPDATE
    print(f"[{sim_id+1}/343] Completed {cell_name} | Slew: {slew_ns}ns | Load: {c_load_pf}pF")
    return results
...
print(f"Starting {len(tasks)} simulations using 4 safe CPU threads...")
all_results = []

# Using 4 workers to prevent RAM overload
...
print("DONE! Check nldm_tables.csv for your fully compiled data.")
